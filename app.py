...
class EmailAssistantApp:
    def __init__(self):
        ...
        try:
            self.model, self.tokenizer = self.load_specialist_model()
    ...
    # --- 2. Model Loading ---
    @st.cache_resource
    def load_specialist_model(_self=None):
    ...
    def planner(self, user_request: str, incoming_message: str) -> dict:
        prompt = (
        ...
        if st.button("Generate Reply", type="primary"):
            if not incoming_message or not user_request:
                ...
                with st.spinner("Agent is thinking and drafting a reply..."):
                    generated_plan = self.planner(user_request, incoming_message)
                    self.executor(generated_plan)
